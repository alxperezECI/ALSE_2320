...
bool ordenar(complejo c[], int tamano)
{
	// metodo burbuja
	bool band;
	do
	{
		...
		{
			complejo aux;
			if (angulo_complejo(c[i]) < angulo_complejo(c[i + 1]))
			{
			...
			else if (norma_complejo(c[i]) < norma_complejo(c[i + 1]))
			{
				c[i] = c[i + 1];
				c[i + 1] = aux;
				band = true;
